...
- This algorithm in general does not offer the fastest service.
- Explain with an example then.
** SSTF Scheduling Algorithms
- Shortest Seek Time First
- Services all the requests near to the current head position before
  moving head far away to sevice other requests.
- This scheduling method results in a total less head movement
  compared to the FCFS scheduling.
** SCAN Scheduling Algorithms
- This approach works like an elevator does.
- It scans down towards the nearest end and then when it hits the
  bottom it scans up servicing the requests that it didn't get going
  down.
- If a request comes in after it has been scanned it will not be
  serviced until the process comes back down or moves back up.
...
- It too jumps to the other end but not all the way to the end. Just
  to the furthest request.
** TODO LOOK Scheduling Algorithms
